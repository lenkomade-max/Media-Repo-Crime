 ...
 */
export function buildEffectsFilter(input, baseLabel = "[0:v]") {
    const effs = input.effects || [];
    if (!effs.length)
        return { filter: "", finalLabel: baseLabel };
    const W = input.width;
    ...
    for (const e of effs) {
        ...
        if (e.kind === "vhs") {
            ...
            chains.push(`${current}format=yuv420p,` +
        ...
        if (e.kind === "retro") {
            ...
            chains.push(`${current}eq=saturation=${saturation}:contrast=${contrast}:gamma=${gamma}:enable='between(t,${ts},${te})',` +
