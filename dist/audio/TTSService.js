 ...
 */
export async function resolveVoiceTrack(input, workDir) {
    if (input.voiceFile)
    ...
    if (input.tts.provider === "kokoro") {
        ...
        if (!resp.ok)
            throw new Error(`Kokoro TTS failed: ${resp.status} ${await resp.text()}`);
    ...
    if (!resp.ok)
        throw new Error(`OpenAI TTS failed: ${resp.status} ${await resp.text()}`);
    const buf = new Uint8Array(await resp.arrayBuffer());
    await fs.writeFile(outPath, buf);
    return outPath;
}
