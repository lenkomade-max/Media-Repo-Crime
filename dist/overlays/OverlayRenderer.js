...
export function buildForceStyle(style) {
    ...
    if (style.size)
        parts.push(`FontSize=${style.size}`);
    if (style.color)
        parts.push(`PrimaryColour=${style.color}`);
    if (style.background)
        parts.push(`BackColour=${style.background}`);
    if (style.outline) {
        if (style.outline.width)
            parts.push(`Outline=${style.outline.width}`);
        if (style.outline.color)
            parts.push(`OutlineColour=${style.outline.color}`);
    }
    if (style.alignment) {
 ...
 */
export async function buildVideoOverlayFilter(input, workDir, baseLabel, srtPath, subStyle) {
