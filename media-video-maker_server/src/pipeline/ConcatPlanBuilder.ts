...
export async function buildSlidesVideo(input: PlanInput, workDir: string): Promise<string> {
  ...
  /**
   * Обработка фото файла - конвертируем в видео с заданной длительностью
   */
  async function handlePhoto(m: MediaFile): Promise<string> {
    const sourcePath = path.resolve(m.src);
    const duration = typeof m.durationSec === "number" ? m.durationSec : input.durationPerPhoto;
    const out = path.join(workDir, `${m.id}_video.mp4`);
    
    // Конвертируем изображение в видео с заданной длительностью
    await runFFmpeg([
      "-y",
      "-loop", "1",
      "-i", sourcePath,
      "-t", String(duration),
      "-vf", `scale=${input.width}:${input.height}:force_original_aspect_ratio=decrease,pad=${input.width}:${input.height}:(ow-iw)/2:(oh-ih)/2:black`,
      "-r", String(input.fps),
      "-c:v", "libx264",
      "-pix_fmt", "yuv420p",
      out
  ...
  for (const m of input.files) {
    try {
      ...
      if (m.type === "video") {
        ...
        // Для видео файлов duration не нужен - длительность уже встроена
        lines.push(`file '${processedFile.replace(/'/g, "'\\''")}'`);
      }
      ...
      if (m.type === "video") {
        ...
        // Для fallback тоже конвертируем в видео
        const duration = typeof m.durationSec === "number" ? m.durationSec : input.durationPerPhoto;
        const fallbackVideo = path.join(workDir, `${m.id}_fallback_video.mp4`);
        await runFFmpeg([
          "-y",
          "-loop", "1",
          "-i", fallbackFile,
        ...
        lines.push(`file '${fallbackVideo.replace(/'/g, "'\\''")}'`);
      }
    }
  }

  // НЕ ДОБАВЛЯЕМ дубликат - теперь все файлы уже видео с встроенной длительностью
