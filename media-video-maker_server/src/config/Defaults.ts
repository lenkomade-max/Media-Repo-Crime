...
export type DefaultsConfig = {
  version: string;
  canvas: { width: number; height: number; fps: number; safeAreaPct: number };
  subtitles: {
    font: string; size: number; color: string; background: string;
    outline: { enabled: boolean; width: number; color: string };
    alignment: "top"|"bottom"; marginV: number;
  };
  overlayTop: {
    font: string; size: number; color: string; background: string;
    outlineWidth: number; anchor: "top"; defaultDurationSec: number;
  };
  overlayBottom: {
    font: string; size: number; color: string; background: string;
    outlineWidth: number; anchor: "bottom"; defaultDurationSec: number;
  };
  shapes: {
    rect: { color: string; thickness: number; fillOpacity: number };
    circle: { color: string; thickness: number; fillOpacity: number; radius: number };
    arrow: { color: string; thickness: number; headSize: number };
  };
  paths: { fontFile: string };
};

const FALLBACK: DefaultsConfig = JSON.parse(`{
  "version":"1",
...
export async function loadDefaults(projectRoot = process.cwd()): Promise<DefaultsConfig> {
